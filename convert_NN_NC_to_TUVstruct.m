...
time.units=ncreadatt(file,'time','units');%removed 20180813 HJR
...
tot.u=ncread(file, 'u', [I(1) J(1) time_index], [I(end)-I(1)+1 J(end)-J(1)+1  length(dtime)]);
tot.v=ncread(file, 'v', [I(1) J(1) time_index], [I(end)-I(1)+1 J(end)-J(1)+1  length(dtime)]);
...
if numel(d)>2
    TUV.U=reshape(tot.u,[d(1)*d(2) d(3)]);
    TUV.V=reshape(tot.v,[d(1)*d(2) d(3)]);
else
